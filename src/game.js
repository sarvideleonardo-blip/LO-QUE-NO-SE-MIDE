...
function getSupabaseConfig() {
  return window.ZOMBIES_LUNARES_SUPABASE || null;
}

function setSupabaseStatus(value) {
  if (ui.supabaseStatus) ui.supabaseStatus.textContent = value;
}

async function publishRunResult(outcome) {
  if (state.resultPublished) return;
  state.resultPublished = true;

  const config = getSupabaseConfig();
  if (!config?.url || !config?.publishableKey || !config?.runsTable) {
    setSupabaseStatus('sin config');
    return;
  }

  setSupabaseStatus('guardando');
  ...
  try {
    const response = await fetch(`${config.url}/rest/v1/${config.runsTable}`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      setSupabaseStatus('tabla no lista');
      console.warn('No se pudo guardar la partida en Supabase:', response.status, await response.text());
      return;
    }

    setSupabaseStatus('guardado');
  } catch (error) {
    setSupabaseStatus('offline');
    console.warn('Supabase no está disponible:', error);
  }
}
