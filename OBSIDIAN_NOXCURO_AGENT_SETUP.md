...
## 9) Solución al error 401 ("Missing Authentication header")

Si ves este error en OpenRouter:

- `401 Missing Authentication header`

entonces la petición salió sin `Authorization: Bearer <API_KEY>`.

Checklist rápido:

1. No subas claves al repo. Usa variable de entorno:

```bash
export OPENROUTER_API_KEY="tu_api_key"
```

2. Usa `scripts/config.example.json` como base para `config.json` y deja `api_key` vacío si usas variable de entorno.

3. Ejecuta el agente de ejemplo:
...
El script `scripts/noxcuro_agent.py` ya envía el header correcto y además corrige el typo común `00imbox` -> `00inbox` para el inbox.
