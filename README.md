...
## Guardar partidas en Supabase

El prototipo ya incluye la URL y la publishable key que compartiste en `src/supabase-config.js`. Al terminar una partida, el juego intenta guardar el resultado en la tabla `zombies_lunares_runs` usando la REST API de Supabase.

Para activar el guardado:

1. Abre el SQL Editor de Supabase.
2. Ejecuta el contenido de `supabase_schema.sql`.
3. Vuelve a jugar una partida. El HUD mostrará `Supabase: guardado` si el insert fue exitoso.

Si la tabla todavía no existe o las policies no están listas, el juego sigue funcionando offline y el HUD mostrará `Supabase: tabla no lista`.
